...
def handle_follow(event):
    ...
    try:
        ...
        # ユーザータイプを判定（初期は未分類）
        user_management_service.set_user_type(user_id, UserType.UNKNOWN)
        
        # ユーザータイプ選択の案内メッセージを送信
        welcome_message = TextSendMessage(
            text=f"👋 {user_name}さん、薬局シフト管理Botへようこそ！\n\n"
                 f"🏥 このBotは薬局の勤務シフト管理を効率化します。\n\n"
                 f"📋 あなたの利用目的を教えてください：\n\n"
                 f"🏪 【店舗の方】\n"
                 f"• 勤務依頼の送信\n"
                 f"• 薬剤師の自動検索・通知\n"
